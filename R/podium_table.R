...
# ---------------------------------------------------------------------------
deltagere_df <- imap_dfr(deltagere, ~ tibble(
  player_key = as.character(.y),
...
p_table <- ggplot() +
  ...
  geom_text(
    ...
    size = 2.9, colour = "grey15", fontface = "plain"
  ) +
  # Full name column
  geom_text(
    data = text_cols,
  ...
  geom_text(
    ...
    hjust = 0.5, size = 3, colour = "grey20", fontface = "bold"
  ) +
  # Column headers
  geom_text(
    data = headers,
